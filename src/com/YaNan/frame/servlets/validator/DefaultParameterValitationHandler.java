...
public class DefaultParameterValitationHandler implements InvokeHandler{
	...
	public void before(MethodHandler methodHandler) {
		...
		if(!methodIgnoreList.contains(method)) {
			...
			if(groupsAnno == null) 
				groupsAnno = methodHandler.getPlugsProxy().getRegisterDescription().getRegisterClass().getAnnotation(Groups.class);
			//是否为快速认证
			boolean fastValidate = method.getAnnotation(FastValidate.class)!=null;
			if(!fastValidate)
				fastValidate = methodHandler.getPlugsProxy().getRegisterDescription().getRegisterClass().getAnnotation(FastValidate.class)!=null;
