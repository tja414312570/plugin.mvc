...
import com.yanan.framework.token.annotation.AuthenticationGroups;
import com.yanan.framework.token.annotation.Chain;
import com.yanan.framework.token.entity.Failed;
import com.yanan.framework.token.entity.Result;
import com.yanan.framework.token.entity.TokenEntity;
import com.yanan.framework.webmvc.ServletBean;
import com.yanan.framework.webmvc.ServletDispatcher;
...
public class TokenFilter extends HttpServlet implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try {
			Token token = WebTokenManager.getInstance().getToken((HttpServletRequest) request);
			if(token==null)
				token = WebToken.addToken(((HttpServletRequest)request),(HttpServletResponse) response);
			...
			if (servletBean != null) {
				...
				ServletDispatcher servletDispatcher = PlugsFactory.getPluginsInstanceByAttribute(ServletDispatcher.class,
						resourceType);
			...
			if(queryParam!=null)
			 ...
			 if(WebTokenManager.getInstance().match(url)) {
				List<TokenEntity> tokenList = WebTokenManager.getInstance().getTokenEntitys(url);
	...
	public boolean dispatcherNamespace(Token token,TokenEntity tokenEntity, ServletRequest request, ServletResponse response, FilterChain chain,String url) throws Exception{
		...
		//if request url container the url's in Token Entity do chain
			...
			if (WebToken.getToken((HttpServletRequest) request) == null) {
				//获取默认failed 或 默认值
